...
const Reports = () => {
  ...
  const fetchAdmins = async () => {
    try {
      ...
      const adminsData = snapshot.docs.map(
        (doc) => ({ id: doc.id, name: doc.data().name } as Admin)
      );
  ...
  const fetchQueueMembers = async () => {
    ...
    try {
      ...
      const mergedData = membersData.map((member) => ({
        ...
        queueName:
          queues.find((queue) => queue.id === member.queueId)?.name ||
          "Unknown",
      }));
      ...
      const filteredData = selectedAdmin
        ? mergedData.filter(
            (member) =>
              lots.find((lot) => lot.id === member.lotId)?.adminId ===
              selectedAdmin
          )
        : mergedData;
  ...
  const filteredMembers = queueMembers.filter(
    (member: any) =>
    ...
    <Card className="p-6 m-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-5 gap-4 mb-4">
        ...
        <Select onValueChange={setSelectedQueue} value={selectedQueue}>
          ...
          <SelectContent>
            {queues
              .filter((queue) => !selectedLot || queue.lotId === selectedLot)
              .map((queue) => (
        ...
        <Popover>
          ...
          <PopoverContent className="w-auto p-0">
            <Calendar
              ...
              selected={endDate}
              onSelect={setEndDate}
              disabled={(date) => !!startDate && date < startDate} // Ensure it always returns a boolean
              initialFocus
            />
          </PopoverContent>
        </Popover>
